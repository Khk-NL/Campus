...
服务器代码部署到 `/opt/campus/source` 后，用户可以自行在远程终端运行 `sudo bash /opt/campus/source/deploy/configure-ai.sh`。脚本隐藏 Key 输入，将它保存到私有环境文件并重启网关；终端不打印 Key。随后仍须用普通已验证账号发起一次真实提问，服务启动成功不等于模型调用成功。

2026-09-26 已完成生产安装：独立 Node 22.23.3、PocketBase 0.40.4、七项迁移、两个 systemd 服务和 Caddy HTTPS 路由。配置备份保存在 `/etc/caddy/Caddyfile.campus-20260926.bak`。官网、隐私页、流程图、后台和健康接口返回 200，未登录 AI 请求返回 401；原网站仍返回 302。服务仅监听 127.0.0.1，没有开放 8090/8787 公网端口。

下一步按 [最终填写流程](FINAL_CONFIGURATION.md) 设置生产管理员、Brevo 发件人及 SMTP 凭据、ChatECNU Key。邮件投递与真实模型尚未验收，旧 APK 也不是新公网配置包。EduWork 成果生成仍需开发适配，不能把模型问答视为完整 EduWork 接入。
