...
完整 PocketBase MVP 的运行步骤与数据迁移边界见 [MVP 说明](../../docs/POCKETBASE_MVP.md)。
...
2. 使用 `users` 集合中的普通试点账号；`_superusers` 管理员账号不能用于移动端。若原库只有管理员，则需要另建普通用户且不能复用管理员密码。不要在仓库中保存试点密码。
3. 调试版在 Android 模拟器启动 Flutter 时设置 `--dart-define=POCKETBASE_URL=http://10.0.2.2:8090`（模拟器访问宿主机）；调试版允许这一本机明文连接。正式 APK 和真机应使用可访问的 HTTPS 地址。未设置该参数时，现有本地课程空间行为保持不变。
