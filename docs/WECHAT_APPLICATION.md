...
用户打开 Campus，进入服务入口、课程空间或学生项目发现。服务入口跳转到原网站，由原服务管理登录；个人课程空间通过邮箱注册、邮件验证和登录，访问当前账号的 PocketBase 数据。用户主动选中资料提问后，网关验证账号与资料归属，再调用 ChatECNU 并返回答案。微信小程序唤起需完成开放平台配置，当前不应描述为已经通过审核或已可用。
