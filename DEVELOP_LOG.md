...
## 2026-09-21 · 生态：开源仓库入口（用户要求），并记录小程序被备案卡住

**状态 / status**：小程序 AppID **暂时申请不到**——用户说明：开放平台的移动应用需要
**App 备案**，而备案要求应用具备一定规模。因此小程序那一步**不是"等凭据"，而是"等产品长大"**，
短期内不会有输入。本轮起转向生态，且用户在 14:00 截止（避开模型高峰），因此只做能独立完成的一块。

### 57. 用户的新指示

> 「还不能申请到 AppID，申请需要 App 备案，而备案需要 app 具有一定规模……你先完成其他功能吧。
> 重点完善我"生态"的构建。14:00 自动停止。」

### 58. 本轮做了什么：开源仓库变成可见入口

用户此前说过「学生应用如果愿意开源可以给出 GitHub 链接」。但 `repositoryUrl` 此前只出现在
**详情弹层**里——也就是要判断"这个学生项目靠不靠谱"，得先点进详情。

现在它是一条**行内可见入口**：挂了仓库的学生应用多一个代码图标按钮，点开用系统浏览器打开
仓库；学校服务**没有**这个按钮（服务根本没有仓库字段），因此不会出现指向空地址的按钮。
测试同时断言这两件事——只断言"学生应用有"会漏掉"服务也冒出一个空链接"这种错误。

### 59. 为什么没做更多

14:00 截止。一条能独立完成、能验证、能提交的改动，好过一条半成品：本轮**没有**动数据库
（PostgreSQL 当时未运行），也**没有**再动小程序（那一步已明确要等备案）。
...
| --- | --- |
| `flutter analyze` | No issues found |
| `flutter test` | **89/89**（断言加在既有测试里，项数不变） |
| 后端 / 数据库 | 本轮未涉及（PostgreSQL 未运行，未启动） |
| 真机截图 | 未做：14:00 前只剩一次构建+安装的时间，我选择不做半途而废的取证 |

### 61. 生态的下一步（按 `CAMPUS_APP_SCHEMA_DESIGN.md` 的阶段表）

| 阶段 | 内容 | 前置 |
| --- | --- | --- |
| S4 | 投稿—审核（`campus_app_submissions` / `review_checks`） | **需要用户先定两件事**：投稿入口是站内表单还是 GitHub Issue；谁能审核（§17 定的是 `moderator`）。另外 `apps/admin` 目前完全不存在，而审核没有后台无法运转 |
| S5 | 人工精选（`campus_app_features`）+ 失效检测（`campus_app_health_checks`） | 不需要凭据。两条纪律要先想清楚：**失效条目留在列表里当墓碑**；**小程序无法自动探测，必须显式标注"无法自动核实"** |
| S6 | 点赞 / 反馈 / 私有备注 | 点赞与反馈需要登录态（Phase 6）；私有备注是本地的，可以先做 |

> 建议顺序：**S5 先做**（不依赖任何外部输入，且"失效检测"直接决定生态的可信度），
> 同时请你回答 S4 的两个问题，之后再做 S4 的后台。
